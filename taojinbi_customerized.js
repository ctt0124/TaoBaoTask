...
function autoRequestScreenCapture(landscape) {
//自定义按钮文本
let capture_permission_button="";
let countDown = new java.util.concurrent.CountDownLatch(1)
let confirmWaitingThread = threads.start(function () {
let confirmWidget = textMatches('START NOW|立即开始|允许').findOne(1000);
if (confirmWidget) {
sleep(200)
let remember = id("android:id/checkbox").findOne(200);
if (!remember) {
remember = id("com.android.systemui:id/remember").findOne(200);
}
if (remember) {
log('找到了记住按钮，点击记住')
click(remember.bounds().centerX(),remember.bounds().centerY())
sleep(200)
} else {
log('未找到记住按钮')
}
...
log(['未找到允许截图按钮，查找内容为：{}', capture_permission_button || 'START NOW|立即开始|允许'], true)
countDown.countDown()
}
})
...
// 截屏查找图片颜色并单击对应的点
function cs_click(num, rgb, xr, yr, wr, hr, flipup,thres) {
    while (num--) {
        let img = captureScreen()
        if (flipup != undefined) img = images.rotate(img, 180)
		if(thres == undefined) let thres = 8
		let point = findColor(img, rgb, { region: [img.getWidth() * xr, img.getHeight() * yr, img.getWidth() * wr, img.getHeight() * hr], threshold: thres })
        ...
        if (point) {
			...
			toast("find point" + point.x +"," + point.y)
			sleep(1000)
...
function duobao_task(back_reg) {
    ...
    //back(); sleep(800); back(); sleep(800); 
	back(); sleep(800); 
	cs_click(4, '#ff7d44', 0.1, 0.2, 0.5, 0.5, true)
    get_rewards(true)
}
...
function main() {
    autoRequestScreenCapture(false);
    console.show();
...
function solo_baba_farm() {
    ...
    thread = threads.start(function () {
        ...
        if (!text('亲密度').findOne(1000)) {
            ...
            while (text('兑换').findOne(3000)) {
                cs_click(2, '#9d4f29', 0.05, 0.3, 0.05, 0.1); sleep(6000)
            }
        }
        console.show()
        do_baba_farm(true)
...
function solo_baba_farm_in_thread() {
	toast_console("开始种树");
	//requestScreenCapture(false);
	sleep(3000);
	app.launch('com.taobao.taobao');
	...
	if (!text('亲密度').findOne(1000)) {
		...
		btn_x.click(); toast_console('进入芭芭农场主界面..');
		sleep(6000)
		btn_click(text('立即去收').findOne(3000))
		while (text('兑换专享好券').findOne(3000)) {
			toast_console("try to find something");
			cs_click(2, '#925524', 0.05, 0.3, 0.05, 0.05); sleep(6000)
		}
	}
	console.show()
	do_baba_farm(true)
...
function close_recent()
{
	...
	btn_click(text('全部关闭').findOne(1000));
}
